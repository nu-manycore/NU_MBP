...
function rtwFileOnLoad(docObj) {
  ...
  if (RTW_TraceArgs.instance && !RTW_TraceArgs.instance.getUseExternalBrowser()) {
      if (frame = rtwSrcFrame()) {
        var plain_link =  frame.document.getElementById("linkToText_plain");
        if (plain_link) {
            plain_link.href = "matlab:rtwprivate('editUrlTextFile','" + plain_link.href + "')";
        }          
      }
  }        
...
function rtwMainOnLoadFcn(topDocObj,aLoc,aPanel) {
  var loc;
  var aHash="";
  var lastArgs = null;
...
function rtwMainOnLoad() {
  rtwMainOnLoadFcn(document,null,true);
}
...
function rtwMainReload(location) {
  ...
  if (RTW_TraceInfo.instance)
     RTW_TraceInfo.instance.removeHighlighting();  
  rtwMainOnLoadFcn(document,location,true);
}

function rtwMainReloadNoPanel(location) {
  rtwMainOnLoadFcn(document,location,false);
}
...
function hiliteEmlLine(docObj, line) {
    ...
    if (trObj != null) {
	trObj.style.backgroundColor = "#66CCFF";
...
function updateCode2ModelLinks(docObj) {
    ...
    if (webviewFrame) {
        if (RTW_TraceArgs.instance && 
            ...
            hiliteCmd = "javascript:rtwHilite(";
        } else {
