...
static int task_time = 1;
static int task_only = 0;
	   ...
	   vi != blk.var ().end (); vi++)
	{
	  ...
	  if (v.mode () == "output" || v.mode () == "extout")
	      ...
	      else
		{
	      ...
	      std::cout << "    syslog(LOG_INFO, \"";
	      if (task_time)
		std::cout << "[%lu]";
	      std::cout << "ATK2_TNAME_TASK_" << id << "\"(" << id << "):\""
			<< v.name () << ":";
...
main (int argc, char* const* argv)
{
  ...
  while ((c = ::getopt (argc, argv, "vc:b:st")) != -1)
    {
      switch (c)
	{
	case 'v':
